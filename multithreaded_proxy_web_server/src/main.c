...
struct cache_element {
	...
	time_t lru_time_track;  // lru_time_track stores the latest time the element
	                        // is  accesed
...
sem_t seamaphore;  // if client requests exceeds the max_clients this seamaphore
                   // puts the waiting threads to sleep and wakes them when
                   // traffic on queue decreases
// sem_t cache_lock;
...
int handle_request(int clientSocket, struct ParsedRequest* request, char* tempReq) {
	...
	if (ParsedRequest_unparse_headers(request, buf + len,
		...
		// return -1;				// If this happens Still try to send request without
		// header
	}
...
void* thread_fn(void* socketNew) {
	...
	char* buffer = (char*)calloc(
	    MAX_BYTES, sizeof(char));  // Creating buffer of 4kb for a client


	bzero(buffer, MAX_BYTES);  // Making buffer zero
	bytes_send_client =
	    recv(socket, buffer, MAX_BYTES,
	         0);  // Receiving the Request of client by proxy server
	...
	if (temp != NULL) {
		// request found in cache, so sending the response to client from proxy's
		// cache
		...
		// ParsedRequest_parse returns 0 on success and -1 on failure.On success it
		// stores parsed request in
		//  the request
		if (ParsedRequest_parse(request, buffer, len) < 0) {
			...
			if (!strcmp(request->method, "GET")) {
				if (request->host && request->path &&
					...
					bytes_send_client =
					    handle_request(socket, request, tempReq);  // Handle GET request
...
int main(int argc, char* argv[]) {
	...
	int i = 0;  // Iterator for thread_id (tid) and Accepted Client_Socket for
	            // each thread
...
cache_element* find(char* url) {
	// Checks for url in the cache if found returns pointer to the respective
	// cache element or else returns NULL
...
int add_cache_element(char* data, int size, char* url) {
	...
	int element_size = size + 1 + strlen(url) +
	                   sizeof(cache_element);  // Size of the new element which
	                                           // will be added to the cache
	if (element_size > MAX_ELEMENT_SIZE) {
		// sem_post(&cache_lock);
		//  If element size is greater than MAX_ELEMENT_SIZE we don't add the
		...
		cache_element* element = (cache_element*)malloc(
		    sizeof(cache_element));  // Allocating memory for the new cache element
		element->data =
